    ...
    def test_regression_alert_generation(self, integrated_test_environment):
        ...
        with patch.object(Path, 'cwd', return_value=env['temp_path']):
            ...
            with patch.object(gates_system, '_simulate_evaluation_results') as mock_simulate:
                ...
                asr_metrics = ASRMetrics(wer=0.03, cer=0.025, bleu_score=0.75, semantic_similarity=0.8, word_count=25)  # Higher WER
                diarization_metrics = DiarizationMetrics(der=0.08, speaker_count_accuracy=0.85, segment_boundary_accuracy=0.8, speaker_purity=0.85, speaker_coverage=0.85)  # Higher DER
                worse_metrics = EnsembleMetrics(asr=asr_metrics, diarization=diarization_metrics, confidence_calibration=0.7, ensemble_agreement=0.7, processing_time=60.0, total_candidates=15, winner_confidence=0.7)
